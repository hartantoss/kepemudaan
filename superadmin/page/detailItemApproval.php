...
<div id="comparisonModals" class="modal">
  <div class="modal-content row">
    ...
    <div class="col s12">
      <div class="btn blue white-text" onclick="approveData()" style="border:none">Approve</div>
      <div class="btn red white-text" onclick="rejectData()" style="border:none">Reject</div>
      <div class="btn orange white-text" onclick="deleteData()" style="border:none">Delete</div>
    </div>
  </div>
</div>
...
<script>
    ...
    const tableMap = {
      ...
      nib: 'NIB',
      nama_kepeloporan: 'Nama Kepeloporan',
      bidang_kepeloporan: 'Bidang Kepeloporan',
      tahun_mulai_pelopor: 'Tahun Mulai Kepeloporan',
      ...
      for (const key in dataA) {
        ...
        if (tableMap.hasOwnProperty(key)) {
          ...
          if (label === "File Upload") {
            const fileA = dataA[key];
            const fileB = dataB?.[key];
            ...
            html += `
              <tr>
                ...
                <td>
                  ${
                    fileB
                      ? `<a href="${"<?php echo $URL."/imagesAPI/fileFormUpload/"?>"+fileB}" target="_blank">Lihat File</a>`
                      : '-'
                  }
                </td>
                <td>
                  ${
                    fileA
                      ? `<a href="${"<?php echo $URL."/imagesAPI/fileFormUpload/"?>"+fileA}" target="_blank">Lihat File</a>`
                      : '-'
                  }
                </td>
              </tr>
    ...
    function deleteData(){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $.ajax({
            url: '<?php echo $URL; ?>/API/delete' + aprjUrl,
            method: 'POST',
            processData: false,
            contentType: false,
            cache: false,
    ...
    function approveData() {
      Swal.fire({
        title: 'Apakah anda yakin?',
        text: "Data yang anda setujui akan masuk ke visualisasi diagram dan data existing akan digantikan data ini.",
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          ...
          formData.append('token', token);
          formData.append("id", selectedId);
          formData.append("idMain", mainId);
          console.log(selectedId+":"+mainId);
